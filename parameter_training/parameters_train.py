...
#alignment_results_path is the location in which you have a series of folders produced by
#running run_nanomgt_on_sample.py (or the parallel version).

#maps_path is the folder in which you have created mutation maps between the isolate consensus sequences
#by running create_variant_maps.py

#json_info_path in the folder in which your species_training/validation.json files are
#which were generated by running generate_batch_jsons.py

#training_or_validation_extension_json should just be '_training.json', assuming you only want to like on the training set :)
...
output_training_folder = 'clean_test_for_final_script'
...
def run_parameter_search(folders, maf_interval, parameters_interval_search, output_training_folder):
    # Initialize and run initial search
    for maf in maf_interval:
    ...
    for round_number in rounds:
        ...
        # Collect and store results from this round
