...
def generate_spline_json(output_folder, maf_intervals, model_name):
    ...
    for name, data in zip(['cor', 'ii', 'pp', 'np', 'dp'],
        ...
        if len(data) >= 4:  # Check if there are enough points for spline fitting
            ...
            print(f"Not enough data points for {name}. Spline fitting skipped.")
...
def run_round_of_parameter_search(round_number, maf_interval, folders, output_folder,
    ...
    for maf in maf_interval:
        ...
        maf_path = setup_directory(output_folder, maf_folder)
    ...
    for maf in maf_interval:
        ...
        if round == None:
            ...
            json_path = os.path.join(output_training_folder,
                         f"{round}_round_maf_{maf}_average_best_params.json")
