...
def read_and_aggregate_results(base_directory):
    ...
    # List all 'np_search_output_*' directories, assuming directory naming follows similar pattern
    search_dirs = [d for d in os.listdir(base_directory) if d.startswith('np_search_output')]
...
def main():
    ...
    results.to_csv("np_f1_scores.csv", index=False)
