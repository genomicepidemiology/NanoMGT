...
def read_and_aggregate_results(base_directory):
    # Define regex to extract 'mrd', 'cor', and batch number from the folder name and Parameters
    param_regex = re.compile(r"mrd_(\d+\.\d+)_.*?cor_(\d+\.\d+)_")
            ...
            for dir_name in dirs:
                if dir_name.endswith('merged'):
                    ...
                    if os.path.exists(file_path):
                        ...
                        # Extract 'mrd' and 'cor' values and add them as columns
    ...
    # Group by 'batch', 'mrd', and 'cor' and calculate the mean of F1 Score
    result = all_data.groupby(['batch', 'mrd', 'cor'])['F1 Score'].mean().reset_index()
    result.columns = ['batch', 'mrd', 'cor', 'average_f1']
