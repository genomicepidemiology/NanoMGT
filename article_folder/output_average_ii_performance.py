...
def read_and_aggregate_results(base_directory):
    # Define regex to extract 'maf', 'iteration_increase', and batch number from the folder name and Parameters
    param_regex = re.compile(r"maf_(\d+\.\d+)_.*?iteration_increase_(\d+\.\d+)")
            ...
            for dir_name in dirs:
                if dir_name.endswith('merged'):
                    ...
                    if os.path.exists(file_path):
                        ...
                        # Extract 'maf' and 'ii' values and add them as columns
                        df['maf'] = df['Parameters'].apply(lambda x: float(param_regex.search(x).group(1)) if param_regex.search(x) else None)
    ...
    # Group by 'batch', 'maf', and 'ii' and calculate the mean of F1 Score
    result = all_data.groupby(['batch', 'maf', 'ii'])['F1 Score'].mean().reset_index()
    result.columns = ['batch', 'maf', 'ii', 'average_f1']
