...
def read_and_aggregate_results(base_directory):
    # Define regex to extract 'maf', 'pp', and batch number from the folder name and Parameters
    param_regex = re.compile(r"maf_(\d+\.\d+)_.*?pp_(\d+\.\d+)_")
            ...
            for dir_name in dirs:
                if dir_name.endswith('merged'):
                    ...
                    if os.path.exists(file_path):
                        ...
                        # Extract 'maf' and 'pp' values and add them as columns
    ...
    # Group by 'batch', 'maf', and 'pp' and calculate the mean of F1 Score
    result = all_data.groupby(['batch', 'maf', 'pp'])['F1 Score'].mean().reset_index()
    result.columns = ['batch', 'maf', 'pp', 'average_f1']
