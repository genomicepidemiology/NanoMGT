...
def read_and_aggregate_results(base_directory):
    # Define regex to extract 'maf', 'dp', and batch number from the folder name and Parameters
    param_regex = re.compile(r"maf_(\d+\.\d+)_.*?dp_(\d+\.\d+)_")
            ...
            for dir_name in dirs:
                if dir_name.endswith('merged'):
                    ...
                    if os.path.exists(file_path):
                        ...
                        # Extract 'maf' and 'dp' values and add them as columns
    ...
    # Group by 'batch', 'maf', and 'dp' and calculate the mean of F1 Score
    result = all_data.groupby(['batch', 'maf', 'dp'])['F1 Score'].mean().reset_index()
    result.columns = ['batch', 'maf', 'dp', 'average_f1']
