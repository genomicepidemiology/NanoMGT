...
def process_data(csv_file):
    ...
    for (mrd, batch), group in grouped:
        ...
        # Normalize pp_values and f1_snpes to range [0, 1]
        pp_values_normalized = (pp_values - np.min(pp_values)) / (np.max(pp_values) - np.min(pp_values))
        f1_snpes_normalized = (f1_snpes - np.min(f1_snpes)) / (np.max(f1_snpes) - np.min(f1_snpes))

        # Fit a spline to the normalized data points
        spline = UnivariateSpline(pp_values_normalized, f1_snpes_normalized, s=None)
        ...
        # Get the first and last F1 snpe from the normalized data
        first_f1_snpe = f1_dense_normalized[0]
        last_f1_snpe = f1_dense_normalized[-1]
