...
def process_data(csv_file, mrd_value=0.01, threshold=5):  # threshold in percentage
    ...
    for idx, batch in enumerate(range(1, 11)):  # Loop over batches 1 to 10
        ...
        if group is not None:
            ...
            diminishing_point = significant_change[0] + 1 if significant_change.size > 0 else len(pp_dense) - 1

            # Plotting on subplots
            ax = axes[idx // 5, idx % 5]  # Determine the row and column by indexing
            ax.plot(pp_values, f1_sppes, 'bo', label='Original Data')
            ax.plot(pp_dense, f1_dense, 'r-', label='Spline Fit')
            ax.axvline(x=pp_dense[diminishing_point], color='g', label='Diminishing Returns Start')
            ax.set_title(f'Batch: {batch}')
            ax.set_xlabel('pp Values')
            ax.set_ylabel('F1 Sppes')
            ax.legend()
...
def main():
    ...
    mrd_value = 0.05  # Set the constant mrd value
